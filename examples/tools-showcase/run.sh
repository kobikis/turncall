#!/usr/bin/env bash
# Run the tools-showcase example. Required values come from the environment or the
# repo-root .env; anything else passes straight through to setup.py.
# NOTE: also pass --webhook-url https://…  (public URL of webhook_server.py, required)
# Usage: ./run.sh --webhook-url https://… [extra args]
set -euo pipefail
