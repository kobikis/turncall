#!/usr/bin/env bash
# Run the call-transfer example. Required values come from the environment or the
# repo-root .env; anything else passes straight through to setup.py.
# NOTE: also pass --transfer-to +1555…  (required)
# Usage: ./run.sh --transfer-to +1555… [extra args]
set -euo pipefail
