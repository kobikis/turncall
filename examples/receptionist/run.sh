#!/usr/bin/env bash
# Run the receptionist example. Required values come from the environment or the
...
exec "$PY" "$DIR/setup.py" --twilio-number "$TURNCALL_NUMBER" --twilio-number-sid "$TWILIO_PN_SID" --server-url "$PUBLIC_BASE_URL" "$@"
