...
```mermaid
flowchart LR
    P["POST /v1/projects"] --> K["POST /v1/api-keys<br/>(returns tc_... once)"]
    K --> A["POST /v1/agents<br/>draft"]
    A --> PUB["POST /v1/agents/:id/publish<br/>immutable version"]
    PUB --> N["POST /v1/phone-numbers<br/>bind number → agent"]
