...
**Deepgram transcripts are no longer profanity-filtered.** Deepgram's filter rewrites the words it matches rather than tagging them, so one false positive silently altered a transcript you store, analyse, and receive in the `call.ended` payload. Transcripts now carry what the caller actually said. There is no per-agent setting to turn the filter back on — [open an issue](https://github.com/kobikis/turncall/issues) if you need one.
