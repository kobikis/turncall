...
```
Workspace admin authorizes Slack once  ──►  held by Pipedream, never by TurnCall
                                              │
Agent attaches it, and chooses when ──────────┤
                                              │
        during a call ──► mcp_servers entry, credentials minted per call
    when a call ends ──► one action, run by the agent's own backend
```
