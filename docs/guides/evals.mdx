...
## Start from a real call

Evals only find what someone thought to test. Turn a call that already happened into a draft:
...
The caller's utterances become turns, the agent's replies become expectations, and every tool the agent called becomes both an expectation and a mock seeded with what that tool actually returned — so the draft is safe to run straight away.
