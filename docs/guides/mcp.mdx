...
<Warning>
  SMS and chat sessions do **not** call tools. `services/llm_text.py` sends no
  `tools` array, so `mcp_servers` on an agent is ignored on those channels —
  the agent will answer from its prompt and knowledge bases only.
</Warning>
