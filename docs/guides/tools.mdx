...
| | Behaviour |
|---|---|
| `sync` (default) | The in-flight call is cancelled the moment the caller speaks over the agent |
| `async` | The call runs to completion and its result is delivered whenever it arrives |

Use `async` for work that is slower than the conversation — a CRM lookup, an
...
<Note>
  ...
  to deliver a late result into, so `execution_mode` is ignored on SMS, the
  Chat API and WhatsApp text.
</Note>
