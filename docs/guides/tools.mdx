...
Tool calls made **during a voice call** (webhook + MCP + built-in) are recorded in the `tool_invocations` table with:
- Input arguments
- Output result
...
<Warning>
  Tool calls in **text** sessions (SMS, Chat API, WhatsApp text) are not
  recorded. The table is keyed by call id and a chat session has none, and the
  stored chat history keeps only the customer and assistant text — so a text
  tool call leaves no trace on either side. Log what you need from your own
  webhook endpoint until that changes.
</Warning>
