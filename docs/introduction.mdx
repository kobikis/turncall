...
<CardGroup cols={2}>
  ...
  <Card title="Speech-to-Speech" icon="bolt">
    ~300ms processing (vs ~800ms cascade) via OpenAI Realtime, Gemini Live or Amazon Nova Sonic.
  </Card>
  <Card title="Tool Calling" icon="wrench">
    Built-in tools (transfer, handoff, DTMF) + webhook tools + MCP servers.
  </Card>
  <Card title="Connected Apps" icon="puzzle-piece">
    Slack and Google Calendar on an agent without handling a token — during a call, or when one ends.
  </Card>
  ...
  <Card title="Takeaways" icon="clipboard-check">
    Define a JSON schema once, attach it to any agent, and get it filled from every call.
  </Card>
  <Card title="Evals" icon="vial">
    Behavioural tests that run against the real pipeline — scripted or simulated, text or audio, with tools mocked. Gate a PR on whether your agent still behaves.
  </Card>
  ...
  <Card title="Observability" icon="chart-line">
    OpenTelemetry traces and pipeline observers — per-stage latency, turn timing, startup.
  </Card>
</CardGroup>
...
flowchart LR
    ...
    MODE -->|"cascade (default)"| C["STT → turn detection → LLM → TTS<br/><b>~800 ms</b> processing · every stage swappable"]
    MODE -->|"s2s"| S["OpenAI Realtime · Gemini Live<br/>Amazon Nova Sonic 2<br/><b>~300 ms</b> processing · one model, no seams"]
    C --> OUT["🔊 Audio back"]
    S --> OUT
